...
% label = v(originalimg, cleanimg, 8, 297, 200);
% label

% originalimg = imread('Case2-Front2.jpg');
% cleanimg = preprocess(originalimg);
% label = v(originalimg, cleanimg, 10, 5010, 3000);
% label

% originalimg = imread('Case2-Rear1.jpg');
% cleanimg = preprocess(originalimg);
% label = v(originalimg, cleanimg, 29, 340, 200);
% label
...
origialimg = imread('Case2-Rear2.jpg');
label = v(origialimg, cleanimg, 15, 174, 400);
label
...
%intialize layout to show all image in a flow manner
tiledlayout('flow');
...
function label = v(img,bwimg,b,s,o)
...
%croppedimg = imopen(croppedimg,se);
